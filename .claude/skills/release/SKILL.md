...
None required. The bump type is determined automatically. The user may optionally override with `patch`, `minor`, or `major`.
...
2. **Determine bump type from changes:**
   ...
   - Apply these rules to decide the bump type (highest matching tier wins):
     - **`major`** if ANY of the following files were changed:
       - `app.config.ts`
       - `eas.json`
     ...
     - **`minor`** if any commit message starts with `feat:` or `feat(` (new JS features that don't touch native)
     - **`patch`** for everything else (bug fixes, chores, asset changes, config tweaks that don't affect native layer)
   - If the user provided an explicit override, use that instead
